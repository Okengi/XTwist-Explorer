...
def setup_xor_shift_tab(tab: ct.CTkFrame):
    ...
    def show_bit_string_value(frame: ct.CTkFrame, string: str, old_string: str, compare=False):
        h = ct.CTkLabel(frame, text="-", font=base_font)
        h.grid(row=0,column=0, padx=4, pady=4)
        h1 = ct.CTkLabel(frame, text="-", font=base_font)
        ...
        for i, char in enumerate(string):
            ...
            if compare:
                if(old[i]=='1' and char=="0"):
                    l = ct.CTkLabel(frame, text=char, font=base_font, text_color="#5752e3")#,bg_color=col)
                elif(old[i]=='0' and char=="1"):
                    l = ct.CTkLabel(frame, text=char, font=base_font, text_color="#138F00")#,bg_color=col)
                elif(old[i]=='1' and char=="1"):
                    l = ct.CTkLabel(frame, text=char, font=base_font, text_color="#e35b52")#,bg_color=col)
                else:
                    l = ct.CTkLabel(frame, text=char, font=base_font) 
            else: 
                if char == "1":
                    l = ct.CTkLabel(frame, text=char, font=base_font, text_color="#e35b52")#,bg_color=col)
                else:
    ...
    def seed_shift_viusal(_=None):
        ...
        bit_text_frame_1 = ct.CTkFrame(algo_frame, fg_color="transparent", border_width=3, border_color="#f86f35", corner_radius=10)
        bit_text_frame_1.grid(row=1, column=0, columnspan=3, padx=10)
        show_bit_string_value(bit_text_frame_1, bit_string_1, "", False)

        value_label_befor_shift_1 = ct.CTkLabel(algo_frame, text="Seed: "+str(value), text_color="#f86f35", font=base_font)
        value_label_befor_shift_1.grid(row=2, column=0)
        
        shiftby_label_1 = ct.CTkLabel(algo_frame, text=f"Shift a: << {str(XOR_Generator.A)}", font=base_font)
        shiftby_label_1.grid(row=2, column=1)

        value = value ^ (value << XOR_Generator.A ) & XOR_Generator.mask

        value_label_after_shift_1 = ct.CTkLabel(algo_frame, text=str(value), text_color="#046574", font=base_font)
        value_label_after_shift_1.grid(row=2, column=2)

        # 2 Shift -------------------------
        bit_string_2 = str(format(value, "032b"))
        bit_text_frame_2 = ct.CTkFrame(algo_frame, fg_color="transparent", border_width=3, border_color="#046574", corner_radius=10)
        ...
        value_label_befor_shift_2 = ct.CTkLabel(algo_frame, text=str(value), text_color="#046574", font=base_font)
        value_label_befor_shift_2.grid(row=4, column=0)
        
        shiftby_label_2 = ct.CTkLabel(algo_frame, text=f"Shift b: >> {str(XOR_Generator.B)}", font=base_font)
        shiftby_label_2.grid(row=4, column=1)

        value = value ^ (value >> XOR_Generator.B ) & XOR_Generator.mask

        value_label_after_shift_2 = ct.CTkLabel(algo_frame, text=str(value), text_color="#4c3ee8", font=base_font)
        value_label_after_shift_2.grid(row=4, column=2)
        # 3 Shift
        bit_string_3 = str(format(value, "032b"))
        bit_text_frame_3 = ct.CTkFrame(algo_frame, fg_color="transparent", border_width=3, border_color="#4c3ee8", corner_radius=10)
        ...
        value_label_befor_shift_3 = ct.CTkLabel(algo_frame, text=str(value), text_color="#4c3ee8", font=base_font)
        value_label_befor_shift_3.grid(row=6, column=0)
        
        shiftby_label_3 = ct.CTkLabel(algo_frame, text=f"Shift c: << {str(XOR_Generator.C)}", font=base_font)
        shiftby_label_3.grid(row=6, column=1)

        value = value ^ (value << XOR_Generator.C ) & XOR_Generator.mask

        value_label_after_shift_3 = ct.CTkLabel(algo_frame, text="Result: "+str(value), text_color="#b5159d", font=base_font)
        value_label_after_shift_3.grid(row=6, column=2)
        # Bits after 3 Shift
        bit_string_4 = str(format(value, "032b"))
        bit_text_frame_4 = ct.CTkFrame(algo_frame, fg_color=cell_color, border_width=3, border_color="#b5159d", corner_radius=10)
    ...
    def setup_controls(_=None):
        ...
        # a, b ,c Controls
        a_text = ct.CTkLabel(control_frame, text="A:", font=base_font)
        a_text.grid(row=1, column=2, padx=(10, 0))
        a_input = ct.CTkEntry(control_frame,width=50, textvariable=a_entry_value, border_color="#e35b52",fg_color=cell_color)
        a_input.grid(row=1,column=3, padx=(0, 10))

        b_text = ct.CTkLabel(control_frame, text="B:", font=base_font)
        b_text.grid(row=2, column=2, padx=(10, 0))
        b_input = ct.CTkEntry(control_frame,width=50, textvariable=b_entry_value, border_color="#e35b52",fg_color=cell_color)
        b_input.grid(row=2,column=3, padx=(0, 10))

        c_text = ct.CTkLabel(control_frame, text="C:", font=base_font)
        c_text.grid(row=3, column=2, padx=(10, 0))
        c_input = ct.CTkEntry(control_frame,width=50, textvariable=c_entry_value, border_color="#e35b52",fg_color=cell_color)
        c_input.grid(row=3,column=3, padx=(0,10))

        var_aplay_button = ct.CTkButton(control_frame, text="Apply", width=50, fg_color="#e35b52", hover_color="#f6756b", command=parameter_change, font=base_font)
        var_aplay_button.grid(row=4, column=2, padx=(0,5))
        random_parameter_button = ct.CTkButton(control_frame, text="Random", width=50, fg_color="#e35b52", hover_color="#f6756b", command=random_parameter, font=base_font)
        ...
        sample_size_slider_label.grid(row=0, column=0, sticky="e", padx=(10,0))
        sample_size_slider_label = ct.CTkLabel(lFrame, text=":", font=base_font)
        ...
        global seed_entry
        seed_entry = ct.CTkEntry(seed_frame,width=150, textvariable=seed, border_color="#e35b52", height=20,fg_color=cell_color)
        ...
        switch_button = ct.CTkButton(control_frame, text="3D Scatter Plot", font=base_font, command=switch, fg_color="#e35b52", hover_color="#f6756b")
